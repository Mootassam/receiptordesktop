...
export default class kycServicess {
  ...
  async create(data) {
    ...
    try {
      const record = await FuturesRepository.create(data, {
        ...this.options,
        session,
      });
  ...
  async update(id, data) {
    ...
    try {
      const record = await FuturesRepository.update(id, data, {
        ...this.options,
        session,
      });
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await FuturesRepository.destroy(id, {
          ...this.options,
          session,
        });
      }
  ...
  async findById(id) {
    return FuturesRepository.findById(id, this.options);
  }
...
4
  ...
  async _isImportHashExistent(importHash) {
    const count = await FuturesRepository.count(
      {
        importHash,
      },
      this.options
