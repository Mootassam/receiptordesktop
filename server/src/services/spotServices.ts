...
export default class kycServicess {
  ...
  async create(data) {
    ...
    try {
      const record = await SpotRepository.create(data, {
        ...this.options,
        session,
      });
  ...
  async update(id, data) {
    ...
    try {
      const record = await SpotRepository.update(
        id,
        data,
        {
          ...this.options,
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await SpotRepository.destroy(id, {
          ...this.options,
          session,
        });
      }
  ...
  async findById(id) {
    return SpotRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return SpotRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return SpotRepository.findAndCountAll(
      args,
      this.options,
    );
  }
